...
- The other installed mods should be the ones the game was played with, at the same versions. A different mod set deals a different game from the same seed - an updated Steamodded rolls a different boss blind, for one. **The config tab** compares logged mods with the mods loaded in the running game. Missing, Extra, and Versions counts summarize differences; the arrow buttons page through full names and logged/loaded versions. **Start Replay** asks for a second press when they differ; pressing it again replays anyway. Restart Balatro after changing mod files.
